#!/usr/bin/env python3
"""Build Google Cloud Batch jobs for Deep CFR Experiment 29."""
...
WORK_ROOT="/workspace/exp29-{kind}-$TASK_SLOT-$RETRY_ATTEMPT"
REPOSITORY="$WORK_ROOT/repository"
OUTPUT_ROOT="$WORK_ROOT/output"
VENV_ROOT="/tmp/exp29-{kind}-venv-$TASK_SLOT-$RETRY_ATTEMPT"
...
TASK_METADATA="$(python - "$TASK_INDEX" <<'PY' | sed -n 's/^EXP29_TASK_METADATA //p' | tail -n 1
import importlib
import sys
module = importlib.import_module("{MODULE}")
index = int(sys.argv[1])
seeds = tuple(module.PRODUCTION_SEEDS)
print("EXP29_TASK_METADATA", int(seeds[index]), module.task_name(index, seeds))
PY
)"
...
exec bash gcp/run_single_deep_cfr_uniform_36h_trajectory.sh "$CONTROLLER_ACTION"
"""
...
def build_job(args) -> dict:
    ...
    return {
        ...
        "labels": {"experiment": "deep-cfr-exp29-sdcfr36h", "stage": args.kind},
    }
