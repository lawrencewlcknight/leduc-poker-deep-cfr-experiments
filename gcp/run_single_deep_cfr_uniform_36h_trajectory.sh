...
BUILDER="$SCRIPT_DIR/single_deep_cfr_uniform_36h_trajectory_batch.py"
MODULE="experiments.leduc_poker.single_deep_cfr_uniform_36h_trajectory.cloud"

if [[ "$ACTION" == "smoke-local" ]]; then
  SMOKE_OUTPUT="${SMOKE_OUTPUT:-/tmp/deep-cfr-exp29-sdcfr36h-smoke}"
  export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/deep-cfr-exp29-matplotlib}"
  export XDG_CACHE_HOME="${XDG_CACHE_HOME:-/tmp/deep-cfr-exp29-cache}"
...
RUN_ID="${RUN_ID:-exp29-sdcfr36h-$(date -u '+%Y%m%d-%H%M%S')}"
...
TEMP_DIR="$(mktemp -d /tmp/deep-cfr-exp29-batch.XXXXXX)"
...
case "$ACTION" in
  dry-run)
    ...
    cp "$TEMP_DIR/evaluate.json" "$REPO_DIR/exp29_evaluate_job.json"
  ...
  run|resume)
    ...
    echo "Remote Deep CFR Experiment 29 controller submitted: $CONTROLLER_JOB"
