"""Frozen scientific contract for Deep CFR Experiment 30."""
...
EXPERIMENT_ID = 30
EXPERIMENT_NAME = "single_deep_cfr_paper_aligned_36h_trajectory"
PRODUCTION_SEEDS = tuple(EXPERIMENT_29_SEEDS)
...
TRAINING_CONFIG = deepcopy(EXPERIMENT_29_CONFIG)
TRAINING_CONFIG.update(
    {
        "experiment_number": EXPERIMENT_ID,
        "experiment_name": EXPERIMENT_NAME,
        "algorithm": "paper-aligned standalone uniformly weighted SD-CFR",
        "source_configuration": "Experiment 29 with paper-aligned fitting budget",
        ...
        # These are the only learning-rule changes from Experiment 29.
        "num_traversals": 1_500,
        "advantage_network_train_steps": 750,
        "learning_rate": 0.001,
        "reset_advantage_optimizer_each_iteration": True,
        # The average-policy network is not part of standalone SD-CFR.
...
def validate_contract(config: Mapping[str, object], *, smoke: bool) -> None:
    ...
    if str(config["experiment_name"]) != EXPERIMENT_NAME:
        raise ValueError("Experiment 30 has the wrong experiment_name")
    if str(config["primary_sd_cfr_weighting"]) != PRIMARY_WEIGHTING:
        raise ValueError("Experiment 30 requires uniform SD-CFR as primary")
    if tuple(config["sd_cfr_weightings"]) != (
        PRIMARY_WEIGHTING,
        SECONDARY_WEIGHTING,
    ):
        raise ValueError("Experiment 30 must evaluate uniform and linear SD-CFR")
    ...
    if not smoke:
        ...
        for key, expected in required.items():
            if config.get(key) != expected:
                raise ValueError(
                    f"Experiment 30 contract field {key} changed: "
                    f"{config.get(key)!r}"
                )
