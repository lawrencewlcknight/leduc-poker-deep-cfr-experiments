"""Training, deferred exact evaluation, and analysis for Experiment 30."""
...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/deep_cfr_exp30_matplotlib")
os.environ.setdefault("XDG_CACHE_HOME", "/tmp/deep_cfr_exp30_cache")
...
_LOGGER = logging.getLogger("deep_cfr_poker.experiment.paper_aligned_sd_cfr_36h")
...
def run_training_seed(*, seed: int, output_dir: Path, smoke: bool = False) -> dict:
    ...
    if seed not in tuple(map(int, config["seeds"])):
        raise ValueError(f"Seed {seed} is outside the Experiment 30 schedule")
...
def run_evaluation_seed(
    *, seed: int, training_dir: Path, output_dir: Path, smoke: bool = False
    ...
    if not manifest:
        raise RuntimeError("Experiment 30 checkpoint manifest is empty")
...
def _plot_trajectory(
    ...
    set_chart_title(
        ...
        algorithm_variant="Experiment 30 paper-aligned candidate",
